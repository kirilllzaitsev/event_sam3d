...
from event_sam3d.utils.event_utils import get_events_from_samples
import torch
...
class MvsecDataLoader(torch.utils.data.Dataset):
    """Dataloader class for MVSEC dataset."""

    NAME = "MVSEC"

    def __init__(self, config: dict = {}, use_ds=False, ds=None):
        self.config = config
        self.use_ds = use_ds
        self.ds = ds
        ...
        if use_ds:
            ...
            samples = fetch_samples(ds)
            events = get_events_from_samples([s[-2] for s in samples])
            self.events = np.vstack([events["x"], events["y"], events["t"], events["p"]]).T
            self.gray_ts = ds.img_timestamps
    ...
    def set_sequence(self, sequence_name: str, undistort: bool = False) -> None:
        ...
        if self.use_ds:
            self.dataset_files = self.events
            ts = {"left": self.events[:, 2]}
            l_event = {"event": self.events, "gray_ts": self.gray_ts}
        else:
    ...
    def load_calib(self) -> dict:
        ...
        outdoor_K = np.pad(outdoor_K, ((0, 1), (0, 1)), mode="constant", constant_values=0)
        outdoor_K[3, 3] = 1.0
        return {"K": outdoor_K}
